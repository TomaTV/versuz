...
export async function updateEnterpriseLeadStatus(formData) {
  const id = String(formData.get("id") || "");
  const status = String(formData.get("status") || "");
  if (!id || !VALID_LEAD_STATUSES.has(status)) {
  ...
  const { error } = await sb
    ...
    .eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/admin/enterprise-leads");
  return { ok: true };
}

export async function deleteEnterpriseLead(formData) {
  const id = String(formData.get("id") || "");
  if (!id) return { error: "Missing id" };
  const guard = await adminGuard();
  if (guard.error) return guard;
  const { sb } = guard;
  const { error } = await sb.from("enterprise_leads").delete().eq("id", id);
  if (error) return { error: error.message };
  revalidatePath("/admin/enterprise-leads");
  return { ok: true };
}
