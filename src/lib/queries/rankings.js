...
import { SNAPSHOT_SKILLS, SNAPSHOT_CLAUDE_MDS } from "@/lib/fixtures/snapshot";
...
async function getPaginatedItemsInternal(kind = "skill", params = {}) {
  ...
  const sb = createSupabasePublicClient();
  if (!sb) {
    // Same snapshot fallback as the catch-error path below.
    const snapshot = kind === "skill" ? SNAPSHOT_SKILLS : SNAPSHOT_CLAUDE_MDS;
    const mapper = kind === "skill" ? mapSkillRow : mapClaudeMdRow;
    const mapped = snapshot.map(mapper);
    return { items: mapped, total: mapped.length, page: 1, totalPages: 1, dbError: true };
  }
  ...
  // `count: "estimated"` lit pg_class.reltuples (µs) au lieu de faire un
  // seq scan sur 90k+ rows. Sur Supabase Free statement_timeout=3s,
  // l'`exact` count timeoutait régulièrement → result.total = 0 →
  // unstable_cache stockait le vide 60s → tous les visiteurs marketplace
  // voyaient "Too many people" jusqu'au refresh cache. PostgREST upgrade
  // automatiquement en `exact` sous un seuil (< 1000 rows par défaut),
  // donc pour les filtres restrictifs la précision reste exacte.
  let q = sb.from(table).select(select, { count: "estimated" });
  ...
  if (error) {
    ...
    // Fallback : static snapshot of top items (regenerated by hand via
    // `node scripts/generate-snapshot.mjs`). The visitor sees real ranked
    // content during a Supabase hiccup instead of an empty grid +
    // alarmist banner. Filters are best-effort matched against the
    // snapshot — if nothing matches we surface the whole top.
    const snapshot = kind === "skill" ? SNAPSHOT_SKILLS : SNAPSHOT_CLAUDE_MDS;
    const mapper = kind === "skill" ? mapSkillRow : mapClaudeMdRow;
    let snapshotRows = snapshot.slice();
    // Best-effort filters that work on snapshot row shape.
    if (cat && cat !== "all") {
      const col = kind === "skill" ? "category" : "project_category";
      snapshotRows = snapshotRows.filter((r) => r[col] === cat);
    }
    if (params.tier && params.tier !== "all") {
      snapshotRows = snapshotRows.filter((r) => r.tier === params.tier);
    }
    const mapped = snapshotRows.map(mapper);
    return {
      items: mapped,
      total: mapped.length,
      page: 1,
